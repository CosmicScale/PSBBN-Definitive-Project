...
# User Guide
The PSBBN Definitive Project script is essential for unlocking all the new features exclusive to version 2.0 and above. The script require an x86-64 processor and a 64-bit operating system. ARM-based systems, including Apple Silicon, are not supported.
...
```
git clone --branch PSBBN-Definitive-Project-v3.0
```
...
The recommended way to install the PSBBN Definitive Project on Windows 10 and 11 is to use the **PSBBN Launcher For Windows**.
...
Download the **PSBBN Launcher for Windows** [here](https://raw.githubusercontent.com/CosmicScale/PSBBN-Definitive-Project/refs/heads/PSBBN-Definitive-Project-v3.0/scripts/PSBBN-Launcher-For-Windows.ps1) (Right-click and select **Save link as**).
...
**NOTE:**  
Always exit the PSBBN Definitive Project Main Menu by pressing `q` to quit. This ensures the drive is unmounted from WSL. If you are using a drive connected via USB, remember to also eject it from the Windows system tray.
